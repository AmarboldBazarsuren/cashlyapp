...
const StatPill = ({ icon, iconColor, bg, label, value, subValue }) => (
  <View style={styles.statPill}>
    ...
    <View>
      ...
      {subValue ? <Text style={styles.statPillSub}>{subValue}</Text> : null}
    </View>
  </View>
);
...
const LoanRow = ({ item, onPress, isLocked }) => (
  ...
  >
    <View style={[styles.loanRowIcon, { backgroundColor: item.bg }]}>
      ...
      {isLocked && <View style={styles.lockBadge}><Ionicons name="lock-closed" size={12} color="#fff" /></View>}
    </View>
    <View style={styles.loanRowMid}>
      <Text style={styles.loanRowLabel}>{item.label}</Text>
      <Text style={styles.loanRowSub}>{item.sub}</Text>
    </View>
    {!isLocked && item.available && <Ionicons name="chevron-forward" size={18} color="#CBD5E1" />}
...
export default function HomeScreen({ navigation }) {
  ...
  const canPayCreditCheck = () => {
    ...
    const daysSince = Math.floor((Date.now() - new Date(user.creditCheckAttemptAt).getTime()) / 86400000);
    return daysSince >= 14;
  };
  ...
  const handleLoan = (item) => {
    if (!item.available) { Toast.show({ type: 'info', text1: item.label, text2: 'Тун удахгүй нэмэгдэнэ!' }); return; }
    if (user?.kycStatus !== 'approved') { Toast.show({ type: 'info', text1: 'Эхлээд мэдээлэл баталгаажуулна уу' }); navigation.navigate('KYCInfo'); return; }
    if (!user?.creditCheckPaid) { navigation.navigate('CreditCheck'); return; }
    if (totalLimit === 0) {
      Toast.show({ type: 'info', text1: 'Зээлийн эрх шалгагдаж байна', text2: '24-48 цагийн дотор тогтоогдоно', visibilityTime: 4000 });
      return;
    }
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      >
        ...
        {/* ── WALLET CARD ─────────────────────────────────────────────────── */}
        <LinearGradient colors={['#5B5BD6', '#22C7BE']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.walletCard}>
          ...
          <View style={styles.walletTopRow}>
            ...
            <TouchableOpacity onPress={() => setBalanceVisible(v => !v)}>
              <Ionicons name={balanceVisible ? 'eye-outline' : 'eye-off-outline'} size={18} color="rgba(255,255,255,0.75)" />
            </TouchableOpacity>
          </View>
        ...
        >
          ...
          <View style={styles.creditTopRow}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={[styles.creditCaption, isLoansLocked && { color: '#64748B' }]}>Зээлийн эрх</Text>
              {isLoansLocked ? (
                ...
                <>
                  <Text style={styles.creditAmount}>{formatMoney(availableCredit)}₮</Text>
                  <Text style={styles.creditSubtext}>
                    Нийт: {formatMoney(effectiveLimit)}₮{multiplierLabel}
                    {usedLimit > 0 && ` · Ашигласан: ${formatMoney(usedLimit)}₮`}
                  </Text>
                </>
              )}
            </View>
          ...
          {!isLoansLocked && (
            <View style={styles.zmsRow}>
              <Ionicons
                name={zmsLimited ? 'warning-outline' : 'checkmark-circle-outline'}
                size={14}
                color={zmsLimited ? '#FDE68A' : 'rgba(255,255,255,0.8)'}
              />
              <Text style={[styles.zmsText, zmsLimited && { color: '#FDE68A' }]}>
                {totalActiveLoans}/5 идэвхтэй зээл (ZMS шалгуур)
                {zmsLimited ? ' · Хязгаарт ойртсон!' : ''}
              </Text>
            </View>
          )}
        ...
        {totalLimit > 0 && (
          <View style={styles.statsRow}>
            <StatPill
              ...
              label="Credit score"
              value={creditScore || '—'}
              subValue={creditScore > 0 && creditScore < 500 ? `${Math.round(multiplier * 100)}% эрх` : null}
            />
            <View style={styles.statsDivider} />
            <StatPill
              icon="time-outline" iconColor="#F59E0B" bg="#FEF9C3"
              label="Идэвхтэй зээл"
              value={`${totalActiveLoans}/5`}
              subValue={externalLoans > 0 ? `+${externalLoans} гадна` : null}
            />
          </View>
        )}
        ...
        {/* ── KYC BANNER ──────────────────────────────────────────────────── */}
        {user?.kycStatus !== 'approved' && (
          <TouchableOpacity style={styles.kycBanner} onPress={() => navigation.navigate('KYCInfo')} activeOpacity={0.82}>
            <View style={styles.kycBannerLeft}>
              <View style={styles.kycBannerIconWrap}><Ionicons name="shield-checkmark-outline" size={20} color="#5B5BD6" /></View>
              <View>
        ...
        {user?.kycStatus === 'approved' && !user?.creditCheckPaid && (
          ...
          >
            <View style={styles.kycBannerLeft}>
              ...
              <View>
                <Text style={styles.kycBannerTitle}>{canPayCreditCheck() ? 'Зээлийн эрх шалгах' : 'Хүлээнэ үү'}</Text>
                <Text style={styles.kycBannerSub}>
                  {canPayCreditCheck() ? '3000₮ төлж зээлийн эрхээ тогтоох' : `${daysUntilNextAttempt()} хоногийн дараа дахин шалгуулна уу`}
                </Text>
              </View>
            </View>
          ...
          <View style={[styles.kycBanner, { backgroundColor: '#DBEAFE', borderColor: '#3B82F6' }]}>
            <View style={styles.kycBannerLeft}>
              ...
              <View>
                ...
                <Text style={styles.kycBannerSub}>24-48 цагийн дотор тогтоогдоно · Доод хэмжээ 480,000₮</Text>
              </View>
            </View>
          </View>
        )}
        ...
        {!isLoansLocked && creditScore > 0 && creditScore < 500 && (
          <View style={[styles.kycBanner, { backgroundColor: creditScore < 300 ? '#FEF2F2' : '#FFF9EC', borderColor: creditScore < 300 ? '#EF4444' : '#F59E0B' }]}>
            <View style={styles.kycBannerLeft}>
              <View style={[styles.kycBannerIconWrap, { backgroundColor: '#fff' }]}>
                <Ionicons name="alert-circle-outline" size={20} color={creditScore < 300 ? '#EF4444' : '#F59E0B'} />
              </View>
              <View style={{ flex: 1 }}>
          ...
          <View style={styles.card}>
            ...
            {activeLoans.slice(0, 2).map((loan, i) => (
              <TouchableOpacity
                ...
                activeOpacity={0.75}
              >
                <View style={styles.loanItemIcon}><Ionicons name="cash-outline" size={18} color="#5B5BD6" /></View>
                <View style={{ flex: 1 }}>
...
const styles = StyleSheet.create({
  ...
  creditTopRow:    { flexDirection: 'row', alignItems: 'center', marginBottom: 12, gap: 12 },
  ...
  creditAmount:    { fontSize: 30, fontWeight: '900', color: '#fff', letterSpacing: -0.8, marginBottom: 2 },
  ...
  statPillSub:  { fontSize: 10, color: '#EF4444', fontWeight: '500', marginTop: 1 },
  statsDivider: { width: 1, backgroundColor: '#E2E8F0', marginHorizontal: 4 },

  kycBanner:       { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#EEEEFF', borderRadius: 16, padding: 14, marginBottom: 14, borderWidth: 1, borderColor: '#D4D4F8' },
